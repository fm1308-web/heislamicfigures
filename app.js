...
async function boot(){
  ...
  if(his.length){
    ...
    document.getElementById('metaLine').innerHTML=
      `${Math.round(mn)} CE to ${Math.round(mx)} CE · ✦ = free book available · △ = dates assumed for visual · <span style="color:#c9a84c">✦</span> = STUDY SPACE`;
...
function initSlider(){
  ...
  function setYear(yr){
    ...
    document.getElementById('yearClearBtn').classList.add('active');
    updateCentHeaders(); updateCentScrollbar();
    renderAll();
  }
...
function toggleYearSlider(){
  const sec=document.getElementById('yearSection');
  const btn=document.getElementById('yearToggleBtn');
  const isInactive=sec.classList.contains('inactive');
  if(isInactive){
    sec.classList.remove('inactive');
    btn.classList.add('on');
    btn.textContent='📅 YEAR FILTER ON';
  } else {
    sec.classList.add('inactive');
    btn.classList.remove('on');
    btn.textContent='📅 YEAR FILTER';
    clearYear();
  }
}
...
function clearYear(){
  ...
  document.getElementById('yearClearBtn').classList.remove('active');
  updateCentHeaders(); updateCentScrollbar();
  renderAll();
}
...
function _syncSliderUI(){
  ...
  if(activeYear!==null){
    ...
    document.getElementById('yearClearBtn').classList.add('active');
    sec.classList.remove('inactive');
    btn.classList.add('on'); btn.textContent='📅 YEAR FILTER ON';
  } else {
    thumb.style.left='0%'; fill.style.width='0%';
    document.getElementById('yearDisplay').textContent='—';
    document.getElementById('yearClearBtn').classList.remove('active');
    sec.classList.add('inactive');
    btn.classList.remove('on'); btn.textContent='📅 YEAR FILTER';
  }
}
...
function setView(v){
  ...
  if(v==='map'||v==='silsila'||v==='studyroom'||v==='eras'){
...
function focusPersonInTimeline(name){
  ...
  if(activeYear!==null){
    ...
    if(p.dob>activeYear||dod<activeYear){
      ...
      const sec=document.getElementById('yearSection');
      const btn=document.getElementById('yearToggleBtn');
      if(sec){sec.classList.add('inactive');}
      if(btn){btn.classList.remove('on');btn.textContent='📅 YEAR FILTER';}
      document.getElementById('yearDisplay').textContent='—';
      document.getElementById('yearClearBtn').classList.remove('active');
    }
  }
